...
const DEFAULT_GUILD_MISSIONS = [
  { id: "cerco-abissal", title: "Cerco Abissal", description: "Reunir uma party para enfrentar uma ameaça acima do nível médio do grupo.", rarity: "Épico", reward: "220 PO + 180 XP para cada membro aprovado" },
...
const EDITION_REPLACED_COLLECTIONS = new Set(["tutorialSteps", "rulesChapters", "faqEntries"]);

function applyEdition3Defaults() {
  const edition = window.MILLENNIUM_V3_CONTENT;
  if (!edition) return;
  Object.assign(DEFAULT_CONTENT.settings, edition.settings || {}, { seedVersion: SEED_VERSION });
  Object.entries(edition.collections || {}).forEach(([collection, incoming]) => {
    if (!Array.isArray(incoming)) return;
    if (EDITION_REPLACED_COLLECTIONS.has(collection)) {
      DEFAULT_CONTENT[collection] = incoming.map((entry) => ({ ...entry }));
      return;
    }
    const existing = Array.isArray(DEFAULT_CONTENT[collection]) ? DEFAULT_CONTENT[collection] : [];
    const byId = new Map(existing.map((entry) => [entry.id, entry]));
    incoming.forEach((entry) => {
      if (!entry?.id) return;
      byId.set(entry.id, { ...(byId.get(entry.id) || {}), ...entry });
    });
    DEFAULT_CONTENT[collection] = [...byId.values()];
  });
}
...
function defaultCharacter(uid, displayName = "") {
  return {
    ...
    fear: "",
    playerLimits: "",
    currentLocation: "Limiar das Cortinas",
...
function playSound(kind) {
  ...
  try {
    ...
    filter.type = "lowpass";
    filter.frequency.value = kind === "secret" ? 5200 : 3400;
    filter.Q.value = 0.8;
    filter.connect(output);
    output.connect(ctx.destination);
    const tone = (note, start, duration, volume, type = "triangle", endRatio = 0.82) => {
      const osc = ctx.createOscillator();
      const gain = ctx.createGain();
      const at = ctx.currentTime + start;
      osc.type = type;
      osc.frequency.setValueAtTime(note, at);
      osc.frequency.exponentialRampToValueAtTime(Math.max(36, note * endRatio), at + duration);
      gain.gain.setValueAtTime(0.0001, at);
      gain.gain.exponentialRampToValueAtTime(volume, at + Math.min(0.025, duration * 0.18));
    ...
    const noise = (start, duration, volume, frequency = 1400) => {
    ...
    if (kind === "click") {
      tone(1280, 0, 0.045, 0.012, "triangle", 0.7);
      tone(210, 0, 0.065, 0.009, "sine", 0.55);
    } else if (kind === "rolling") {
      noise(0, 0.7, 0.026, 720);
      tone(55, 0, 0.86, 0.038, "sine", 2.8);
      tone(110, 0.08, 0.72, 0.024, "triangle", 2.1);
      tone(220, 0.28, 0.52, 0.014, "sine", 1.7);
    } else if (kind === "rare") {
      tone(196, 0, 0.58, 0.026, "triangle", 0.95);
      tone(293.66, 0.055, 0.72, 0.023, "sine", 1.01);
      tone(440, 0.13, 0.84, 0.018, "sine", 0.99);
      noise(0, 0.18, 0.008, 2500);
    } else if (kind === "legendary") {
      tone(55, 0, 0.72, 0.045, "sine", 0.58);
      [146.83, 220, 293.66, 440].forEach((note, index) => tone(note, 0.08 + index * 0.09, 0.9, 0.025 - index * 0.002, index % 2 ? "sine" : "triangle", 1.02));
      noise(0.02, 0.5, 0.018, 900);
    } else if (kind === "secret") {
      noise(0, 0.86, 0.022, 520);
      tone(41.2, 0, 1.25, 0.05, "sine", 0.72);
      [130.81, 196, 261.63, 392, 523.25].forEach((note, index) => tone(note, 0.18 + index * 0.14, 1.05, 0.024, "sine", 1.01));
    } else if (kind === "fail") {
      tone(146.83, 0, 0.28, 0.018, "triangle", 0.72);
      tone(73.42, 0.04, 0.4, 0.018, "sine", 0.8);
    } else {
      tone(220, 0, 0.18, 0.018, "triangle", 0.9);
      tone(329.63, 0.035, 0.28, 0.012, "sine", 0.94);
    }
    window.setTimeout(() => {
      try { filter.disconnect(); output.disconnect(); } catch {}
    }, 1800);
  } catch {
...
async function startAmbientMusic() {
  ...
  const motifs = {
    awakening: { drone: [73.42, 110, 146.83], melody: [220, 261.63, 293.66, 329.63, 293.66, 246.94, 220, 196], interval: 3200, filter: 840 },
    void: { drone: [55, 82.41, 123.47], melody: [164.81, 196, 233.08, 174.61, 146.83, 130.81], interval: 3900, filter: 620 },
    blood: { drone: [65.41, 98, 130.81], melody: [196, 207.65, 261.63, 233.08, 174.61, 155.56], interval: 3000, filter: 760 },
    frost: { drone: [82.41, 123.47, 164.81], melody: [246.94, 329.63, 369.99, 293.66, 246.94, 220], interval: 3600, filter: 1040 },
  ...
  filter.type = "lowpass";
  filter.frequency.value = motif.filter;
  filter.Q.value = 0.7;
  lfo.type = "sine";
...
function generatedPetPortrait(item = {}) {
  ...
  const name = normalize(item.name || identity);
  const kind = /corvo|coruja|falcao|ibis|abutre|galo|mariposa/.test(name) ? "wing"
    : /lobo|cao|raposa|fenrir|hiena/.test(name) ? "canine"
      : /serpente|escorpiao|aranha|hidra/.test(name) ? "serpent"
        : /peixe|lontra|tartaruga|leviata/.test(name) ? "aquatic"
          : /cervo|carneiro|javali|urso/.test(name) ? "horned"
            : /gato|pantera|lince|leao|grifo/.test(name) ? "feline"
              : /mimico|lesma|besouro|sapo/.test(name) ? "oddity"
                : "wraith";
  const silhouettes = {
    wing: `<path d="M55 246 Q94 112 184 156 Q246 85 329 128 Q287 183 232 198 Q278 224 306 282 Q238 266 192 225 Q140 278 70 292 Q102 231 150 204 Q94 198 55 246Z"/><path d="M177 151 194 78 212 151Z"/>`,
    canine: `<path d="M70 286 Q82 211 134 190 L117 111 165 145 Q195 126 225 145 L274 109 254 193 Q309 220 319 286 L249 260 222 282 161 282 132 258Z"/><path d="M143 194 Q192 157 244 194 L226 242 192 264 156 242Z"/>`,
    serpent: `<path d="M74 286 Q132 256 109 207 Q91 158 141 126 Q198 91 250 124 Q300 156 269 200 Q245 235 286 285 Q234 260 203 281 Q157 304 119 279Z"/><path d="M153 139 Q192 93 233 139 L218 197 192 218 165 197Z"/>`,
    aquatic: `<path d="M48 224 Q112 133 221 158 L311 112 294 198 336 249 245 235 Q135 294 48 224Z"/><path d="M124 162 170 91 205 163Z"/>`,
    horned: `<path d="M82 286 Q95 211 145 178 L112 112 151 133 136 74 184 145 232 73 219 137 273 108 239 181 Q292 214 302 286Z"/><path d="M145 178 Q192 140 239 178 L226 241 192 267 158 241Z"/>`,
    feline: `<path d="M70 286 Q86 211 142 183 L124 113 176 145 192 132 208 145 260 113 242 183 Q298 211 314 286Z"/><path d="M145 177 Q192 142 239 177 L226 235 192 260 158 235Z"/>`,
...
async function deleteDoc(collection, id) {
  ...
  if (state.demo) {
    ...
    if (state.content?.[collection]) state.content[collection] = state.content[collection].filter((item) => item.id !== id);
    syncPrivateMessages();
    render();
    return;
  }
...
async function addGlobalMessage(data) {
  ...
  const id = await addDoc("globalMessages", payload);
  const pruneSource = state.globalMessages.some((message) => message.id === id)
    ? state.globalMessages
    : [...state.globalMessages, { ...payload, id, createdAt: new Date().toISOString() }];
  await pruneMessages("globalMessages", pruneSource);
}
    ...
    <section class="onboarding-ritual">
      ...
      <div class="onboarding-copy">
        ...
        <div class="onboarding-actions">
          ...
          ${safeIndex < steps.length - 1 ? `<button class="primary-button" type="button" data-action="onboarding-next">Próximo</button>` : `<button class="primary-button intense" type="button" data-action="onboarding-complete">Entrar em Millennium</button>`}
        </div>
      </div>
    </section>
  `;
...
async function completeOnboarding() {
  const version = state.settings.onboardingVersion || state.settings.rulesVersion || "3.0";
  await writeDoc("users", state.user.uid, { tutorialSeenVersion: version });
  state.profile = { ...state.profile, tutorialSeenVersion: version };
  closeModal();
  toast("Primeiro Despertar concluído. A Interface agora acompanha sua jornada.");
}
  ...
  return `
    <div class="grid">
      <article class="panel span-12 terms-gate">
        ...
        <h2>Antes de despertar em Millennium</h2>
        ...
        <form class="form-stack" data-form="terms-accept">
          <label class="checkbox-line"><input name="accepted" type="checkbox" value="true" required /> <span>Li e aceito as regras de convivência, integridade, punições e uso da Interface.</span></label>
...
function renderProfileTab(character, totals) {
  ...
  if (tab === "cosmetics") {
    ...
    return `
      <article class="panel span-12 cosmetics-vault-panel">
        <div class="panel-heading"><div><p class="eyebrow">Cofre de aparências</p><h3>Molduras, banners e efeitos</h3></div><span class="tag">${cosmetics.length} cosmético(s)</span></div>
        <p class="muted-text">Equipe uma aparência para ela entrar na prévia pública. Molduras do mesmo tipo substituem a anterior; nada é destruído.</p>
        <div class="cosmetics-grid">
          ${cosmetics.map((item) => `<article class="cosmetic-card ${rarityClass(item.rarity)} ${item.equipped ? "equipped" : ""}"><div class="cosmetic-preview"><span>${/moldura/i.test(item.name) ? "◇" : /banner/i.test(item.name) ? "▰" : "✦"}</span></div><small>${esc(item.category || item.categoryId || "Cosmético")} · ${esc(item.rarity || "Comum")}</small><strong>${esc(item.name)}</strong><p>${esc(item.description || item.effect || "Aparência registrada no Cofre.")}</p><button class="ghost-button" type="button" data-action="toggle-equip" data-item-instance="${esc(item.instanceId || item.id)}">${item.equipped ? "Desequipar" : "Equipar"}</button></article>`).join("") || `<div class="empty-state">Cosméticos do Passe, fragmentos, eventos e Invocação aparecem aqui.</div>`}
        </div>
      </article>
    `;
  }
    ...
    <article class="panel span-12 faction-pledge-panel">
      ...
      ` : state.settings.factionsUnlocked ? `
        ...
        <form class="form-stack faction-pledge-form" data-form="faction-pledge">
          <label><span>Facção</span><select name="factionId" required><option value="">Selecione...</option>${state.content.reputationFactions.map((entry) => `<option value="${esc(entry.id)}">${esc(entry.name)} · ${esc(entry.region || "Millennium")}</option>`).join("")}</select></label>
...
function renderLegacyCharacterForm() {
  ...
  const visibleBase = Object.fromEntries(ATTRIBUTES.map((attr) => [attr.key, Number(draftValue(draft, `base_${attr.key}`, character.base?.[attr.key] ?? (locked ? 0 : 4)))]));
  ...
  return `
    <form class="grid character-forge" data-form="character">
      <article class="panel span-12 character-record-head">
        <div class="panel-heading">
          <div>
            <p class="eyebrow">Registro do Escolhido</p>
      ...
      <article class="panel span-7 character-section">
        <div class="section-title"><span>01</span><div><p class="eyebrow">Quem retornou</p><h3>Identidade e origem</h3></div></div>
        <div class="form-grid">
          ...
          <label><span>Pronomes</span><input name="pronouns" value="${esc(draftValue(draft, "pronouns", character.pronouns || ""))}" placeholder="Ex.: ele/dele" /></label>
          ...
          <label><span>Reino atual</span><select name="kingdomId"><option value="">Sem pátria registrada</option>${kingdoms.map((entry) => `<option value="${esc(entry.id)}" ${draftValue(draft, "kingdomId", character.kingdomId || "") === entry.id ? "selected" : ""}>${esc(entry.name)}</option>`).join("")}</select></label>
          <label><span>Localização atual</span><input name="currentLocation" value="${esc(draftValue(draft, "currentLocation", character.currentLocation || "Limiar das Cortinas"))}" /></label>
      ...
      <article class="panel span-5 character-section">
        <div class="panel-heading">
          <div>
            <p class="eyebrow">Matriz inicial</p>
      ...
      <article class="panel span-7 character-section">
        <div class="section-title"><span>03</span><div><p class="eyebrow">Núcleo narrativo</p><h3>História, desejo e limites</h3></div></div>
        <div class="form-grid narrative-fields">
          <label class="wide"><span>Descrição pública</span><textarea name="characterDescription" rows="3">${esc(draftValue(draft, "characterDescription", character.characterDescription || ""))}</textarea></label>
          <label class="wide"><span>História relevante</span><textarea name="story" rows="4">${esc(draftValue(draft, "story", character.story || ""))}</textarea></label>
          <label class="wide"><span>Personalidade</span><textarea name="personality" rows="3">${esc(draftValue(draft, "personality", character.personality || ""))}</textarea></label>
          <label><span>Objetivo atual</span><textarea name="objective" rows="3">${esc(draftValue(draft, "objective", character.objective || ""))}</textarea></label>
          <label><span>Medo ou perda</span><textarea name="fear" rows="3">${esc(draftValue(draft, "fear", character.fear || ""))}</textarea></label>
          <label class="wide"><span>Limites do jogador</span><textarea name="playerLimits" rows="3" placeholder="Temas que exigem aviso, consentimento ou não devem aparecer em cenas pessoais.">${esc(draftValue(draft, "playerLimits", character.playerLimits || ""))}</textarea></label>
        </div>
      </article>
      <article class="panel span-12 character-power-record">
        <div class="panel-heading"><div><p class="eyebrow">Registro aprovado</p><h3>Poder central e técnica inicial</h3></div><button class="ghost-button" type="button" data-nav="missions">Enviar criação para análise</button></div>
        <div class="list">
          <div class="item-row">
            <span>Poder</span>
    ...
    <div class="grid">
      <article class="panel span-12">
        <div class="roulette-stage">
          ...
          <div class="affinity-contract-strip">${sortByName(state.content.affinityCategories).map((entry) => `<span style="--affinity-color:${esc(entry.color || "#d8b45d")}"><small>${esc(entry.name)}</small><b>${Number(entry.weight || 0)}%</b></span>`).join("")}</div>
...
function renderInventoryItems(items, withEquip) {
  ...
  return items.map((item) => `
    <div class="item-row">
      <span>${esc(item.rarity || "Comum")}${Number(item.quantity || 1) > 1 ? ` · x${Number(item.quantity || 1)}` : ""}</span>
      <strong>${esc(item.name)}</strong>
  ...
  const views = {
    ...
    terms: () => `
      <article class="panel span-12 terms-gate">
        ...
        <h2>Termo de convivência e uso</h2>
        <p>${esc(state.settings.termsText || DEFAULT_CONTENT.settings.termsText)}</p>
    ...
    <div class="grid">
      <article class="panel span-12">
        <div class="panel-heading">
          ...
          <div class="action-row"><span class="tag">Versão ${esc(state.settings.rulesVersion || "1.0")}</span><button class="primary-button" type="button" data-action="open-onboarding">Iniciar tutorial guiado</button></div>
        </div>
...
function renderGlobalSearchResults() {
  ...
  const results = pools.flatMap(([type, items]) => items.map((item) => ({ type, item })))
    ...
    .slice(0, 40);
  return results.map(({ type, item }) => `<div class="content-card"><span>${esc(type)}</span><h3>${esc(item.name || item.title || item.id)}</h3><p>${esc(item.description || item.summary || item.answer || item.passive || "")}</p></div>`).join("") || `<div class="empty-state">Digite algo para buscar no site.</div>`;
}
...
function filteredMarketListings() {
  const marketQuery = normalize(state.marketSearch || "");
  return (state.content.marketListings || []).filter((item) => {
    const matchesText = !marketQuery || normalize(`${item.name || ""} ${item.description || ""} ${item.rarity || ""}`).includes(marketQuery);
    const matchesRarity = state.marketRarity === "all" || rarityKey(item.rarity) === rarityKey(state.marketRarity);
    const matchesCategory = state.marketCategory === "all" || item.categoryId === state.marketCategory;
    return matchesText && matchesRarity && matchesCategory;
  });
}
...
function renderMarket() {
  ...
  const cards = {
    ...
    crafting: () => `<section class="player-forge-intro"><div><p class="eyebrow">Forja do Escolhido</p><h3>Transforme o que a Hunt devolveu</h3><p>Materiais são consumidos pelo próprio player e o resultado entra imediatamente no inventário. O Oráculo só intervém em disputa, erro ou efeito narrativo sensível.</p></div><span class="tag">Autonomia registrada</span></section>${marketCards(state.content.craftingRecipes, (item) => `${item.materials || "Materiais a definir"} → ${item.result || ""}`, "Crafting")}`,
...
function marketCards(items, detail, currency = "PO") {
  return sortByName(items).map((item) => `
    <div class="content-card market-card">
      ...
      <p>${esc(detail(item))}</p>
      <div class="action-row">
        <button class="primary-button" type="button" data-action="${currency === "PO" ? "buy-market-item" : currency === "Crafting" ? "craft-item" : currency === "Coleção" ? "open-cosmetics" : "request-market-item"}" data-market-id="${esc(item.id)}" data-market-currency="${esc(currency)}">${currency === "PO" ? "Comprar agora" : currency === "Coleção" ? "Abrir meu cofre" : currency === "Crafting" ? "Forjar agora" : "Solicitar"}</button>
...
function renderAuctionMarket() {
  return (state.content.auctionListings || []).map((auction) => {
    ...
    const scheduled = Number.isFinite(endTime);
    const ended = scheduled && Date.now() >= endTime;
    ...
    return `
      <article class="content-card market-card auction-card ${rarityClass(auction.rarity)}">
        ...
        <p class="hint">${scheduled ? `${ended ? "Encerrado em" : "Encerra em"} ${end.toLocaleString("pt-BR")}` : "O Oráculo ainda precisa publicar a data deste leilão."}</p>
        <div class="action-row">
          ${!scheduled ? `<button class="secondary-button" type="button" disabled>Aguardando publicação</button>` : !ended ? `<button class="primary-button" type="button" data-action="open-auction-bid" data-auction-id="${esc(auction.id)}">Dar lance</button>` : winner && !claimed ? `<button class="primary-button" type="button" data-action="claim-auction" data-auction-id="${esc(auction.id)}">Coletar vitória</button>` : `<span class="tag">${claimed ? "Coletado" : winner ? "Aguardando coleta" : "Resultado definido"}</span>`}
        </div>
      </article>
    `;
    ...
    <div class="grid">
      <article class="panel span-12 economy-hero">
        ...
        <div class="stat-grid compact-stat-grid">
          ...
          ${renderStat("Energia no servidor", `${totalEnergy}/${Math.max(1, players.length) * GACHA_ENERGY_MAX}`)}
...
function renderAdminUsers() {
  ...
  return `
    <div class="grid">
      <article class="panel span-4">
        <div class="panel-heading"><div><p class="eyebrow">Players</p><h3>Escolhidos registrados</h3></div></div>
        <div class="scroll-list">
...
const VISUAL_FORGE_COLLECTIONS = new Set([
  "races", "classes", "affinityCategories", "affinities", "itemCategories", "items",
  "biomes", "kingdoms", "regions", "npcs", "rulesChapters", "faqEntries", "tutorialSteps",
  "gachaPets", "gachaItems", "gachaShardShops", "towerMaps", "gachaBanners",
  "marketListings", "auctionListings", "craftingRecipes", "worldLore", "worldEvents",
  "campaignPosts", "reputationFactions", "bestiary",
]);
...
function forgeVisualFields(collection, item = {}) {
  ...
  if (collection === "classes") {
    return `
      <label><span>Nome da classe-base</span><input name="name" value="${esc(item.name || "")}" required /></label>
      ${mediaInput("imageUrl", "Imagem da classe", item.imageUrl || "")}
      <fieldset class="forge-fieldset wide"><legend>Bônus de atributo</legend>${forgeBonusInputs(item)}</fieldset>
      <label class="wide"><span>Papel narrativo</span><textarea name="role" rows="3">${esc(item.role || "")}</textarea></label>
      <label class="wide"><span>Caminhos avançados</span><input name="paths" value="${esc(Array.isArray(item.paths) ? item.paths.join(", ") : item.paths || "")}" placeholder="Cavaleiro do Eclipse, Guardião do Véu..." /></label>
      <label class="wide"><span>Descrição</span><textarea name="description" rows="4">${esc(item.description || "")}</textarea></label>`;
  }
  if (collection === "affinityCategories") {
    return `
      <label><span>Nome da categoria</span><input name="name" value="${esc(item.name || "")}" required /></label>
      ${mediaInput("imageUrl", "Símbolo da categoria", item.imageUrl || "")}
      <label><span>Peso na roleta</span><input name="weight" type="number" min="0" step="0.1" value="${Number(item.weight || 0)}" /></label>
      <label><span>Raridade-base</span>${forgeRaritySelect(item.rarity || "Comum")}</label>
      <label><span>Cor ritual</span><input name="color" type="color" value="${esc(item.color || "#d8b45d")}" /></label>
      <label class="wide"><span>Regra e descrição</span><textarea name="description" rows="4">${esc(item.description || "")}</textarea></label>`;
  }
  if (collection === "affinities") {
    return `
      <label><span>Nome da afinidade</span><input name="name" value="${esc(item.name || "")}" required /></label>
      ${mediaInput("imageUrl", "Arte da afinidade", item.imageUrl || "")}
      <label><span>Categoria</span><select name="categoryId">${(state.content.affinityCategories || []).map((category) => `<option value="${esc(category.id)}" ${item.categoryId === category.id ? "selected" : ""}>${esc(category.name)}</option>`).join("")}</select></label>
      <fieldset class="forge-fieldset wide"><legend>Bônus de atributo</legend>${forgeBonusInputs(item)}</fieldset>
      <label class="wide"><span>Domínio e passiva</span><textarea name="passive" rows="4">${esc(item.passive || "")}</textarea></label>
      <label class="wide"><span>Descrição, custo e limites</span><textarea name="description" rows="5">${esc(item.description || "")}</textarea></label>`;
  }
  if (collection === "itemCategories") {
    return `<label><span>Nome da categoria</span><input name="name" value="${esc(item.name || "")}" required /></label>`;
  }
  if (collection === "items") {
    return `
      <label><span>Nome do item</span><input name="name" value="${esc(item.name || "")}" required /></label>
      ${mediaInput("imageUrl", "Imagem do item", item.imageUrl || "")}
      <label><span>Categoria</span><select name="categoryId">${(state.content.itemCategories || []).map((category) => `<option value="${esc(category.id)}" ${item.categoryId === category.id ? "selected" : ""}>${esc(category.name)}</option>`).join("")}</select></label>
      <label><span>Preço em PO</span><input name="price" type="number" min="0" value="${Number(item.price || 0)}" /></label>
      <label><span>Raridade</span>${forgeRaritySelect(item.rarity || "Comum")}</label>
      <fieldset class="forge-fieldset wide"><legend>Bônus de atributo</legend>${forgeBonusInputs(item)}</fieldset>
      <label class="wide"><span>Descrição, tags e limites</span><textarea name="description" rows="4">${esc(item.description || "")}</textarea></label>`;
  }
  if (collection === "biomes" || collection === "kingdoms" || collection === "regions" || collection === "npcs") {
    const label = collection === "biomes" ? "bioma" : collection === "kingdoms" ? "reino" : collection === "regions" ? "região" : "NPC";
    return `
      <label><span>Nome do ${label}</span><input name="name" value="${esc(item.name || "")}" required /></label>
      ${mediaInput("imageUrl", `Imagem do ${label}`, item.imageUrl || "")}
      ${collection === "biomes" ? `<label><span>Região maior</span><input name="region" value="${esc(item.region || "")}" /></label>` : ""}
      ${collection === "kingdoms" ? `<label><span>Governante</span><input name="ruler" value="${esc(item.ruler || "")}" /></label>` : ""}
      ${collection === "regions" ? `<label><span>Reino</span><select name="kingdomId"><option value="">Independente</option>${(state.content.kingdoms || []).map((kingdom) => `<option value="${esc(kingdom.id)}" ${item.kingdomId === kingdom.id ? "selected" : ""}>${esc(kingdom.name)}</option>`).join("")}</select></label>` : ""}
      ${collection === "npcs" ? `<label><span>Função</span><input name="role" value="${esc(item.role || "")}" /></label><label><span>Região</span><input name="region" value="${esc(item.region || "")}" /></label>` : ""}
      <label class="wide"><span>Descrição</span><textarea name="description" rows="6">${esc(item.description || "")}</textarea></label>`;
  }
  if (collection === "rulesChapters") {
    return `<label><span>Título</span><input name="name" value="${esc(item.name || "")}" required /></label><label><span>Ordem</span><input name="order" type="number" min="1" value="${Number(item.order || 1)}" /></label><label class="wide"><span>Resumo</span><textarea name="summary" rows="3">${esc(item.summary || "")}</textarea></label><label class="wide"><span>Versão completa</span><textarea name="full" rows="8">${esc(item.full || "")}</textarea></label>`;
  }
  if (collection === "faqEntries") {
    return `<label><span>Pergunta</span><input name="name" value="${esc(item.name || "")}" required /></label><label><span>Categoria</span><input name="category" value="${esc(item.category || "FAQ")}" /></label><label class="wide"><span>Resposta</span><textarea name="answer" rows="7">${esc(item.answer || "")}</textarea></label>`;
  }
  if (collection === "tutorialSteps") {
    return `<label><span>Título da etapa</span><input name="name" value="${esc(item.name || "")}" required /></label><label><span>Ordem</span><input name="order" type="number" min="0" value="${Number(item.order || 0)}" /></label><label><span>Área de destino</span><input name="destination" value="${esc(item.destination || "")}" placeholder="Perfil, Codex, Missões..." /></label><label class="wide"><span>Descrição guiada</span><textarea name="description" rows="6">${esc(item.description || "")}</textarea></label><label class="wide"><span>Regra essencial</span><input name="rule" value="${esc(item.rule || "")}" /></label>`;
  }
...
function renderContentEditor() {
  ...
  return `
    <article class="panel span-5">${contentForm("content-item", "Novo item", `
      ...
      <label><span>Raridade</span><select name="rarity">${RARITIES.map((r) => `<option value="${r}">${r}</option>`).join("")}</select></label>
      <fieldset class="forge-fieldset"><legend>Bônus de atributo</legend>${forgeBonusInputs()}</fieldset>
      <label><span>Descrição, tags e limites</span><textarea name="description" rows="4"></textarea></label>
    `)}</article>
      ...
      <article class="panel span-6">
        ...
        <form class="form-stack" data-form="admin-reward">
          ...
          <label><span>Tipo</span><select name="type">
            ...
            <option value="gachaItem">Relíquia para inventário</option>
    ...
    <div class="grid">
      <article class="panel span-7">
        ...
        <form class="form-grid" data-form="admin-mail">
          ...
          <label><span>PO</span><input name="gold" type="number" min="0" value="0" /></label>
          <label><span>Millennium Coins</span><input name="coins" type="number" min="0" value="0" /></label>
          <label><span>Energia</span><input name="energy" type="number" min="0" value="0" /></label>
          <label><span>Essências de afinidade</span><input name="essences" type="number" min="0" value="0" /></label>
          <label><span>Tipo de fragmento</span><select name="fragmentName">${fragmentBalances({ gachaFragments: {} }).map((entry) => `<option value="${esc(entry.name)}">${esc(entry.name)}</option>`).join("")}</select></label>
          <label><span>Quantidade de fragmentos</span><input name="fragmentAmount" type="number" min="0" value="0" /></label>
          ...
          <label><span>Item</span><select name="itemId"><option value="">Nenhum</option>${optionList(state.content.items)}</select></label>
          <label><span>Companheiro dimensional</span><select name="gachaPetId"><option value="">Nenhum</option>${optionList(state.content.gachaPets)}</select></label>
          <label><span>Cosmético ou relíquia</span><select name="gachaItemId"><option value="">Nenhum</option>${optionList(state.content.gachaItems)}</select></label>
          <label><span>Token</span><input name="tokenName" placeholder="Ex.: Testemunha da Fenda" /></label>
          <label><span>Afinidade</span><select name="affinityId"><option value="">Nenhuma</option>${optionList(state.content.affinities)}</select></label>
        ...
        <div class="list">
          ${sortByName(state.content.affinityCategories).map((cat) => `
            <div class="item-row">
              ...
              <p>${Number(cat.weight || 0)}% · ${esc(cat.rarity)} · ${state.content.affinities.filter((entry) => entry.categoryId === cat.id).length} afinidade(s)</p>
            </div>
          `).join("")}
        </div>
      </article>
...
async function saveLegacyCharacter(form) {
  ...
  await updateCharacter(state.user.uid, {
    ...
    characterAge: values.apparentAge === "" ? "" : Math.max(0, Number(values.apparentAge || 0)),
    characterDescription: values.characterDescription || "",
    pronouns: values.pronouns || "",
    originType: values.originType || "Retornado",
    apparentAge: values.apparentAge === "" ? "" : Math.max(0, Number(values.apparentAge || 0)),
    realAge: values.realAge === "" ? "" : Math.max(0, Number(values.realAge || 0)),
    culture: values.culture || "",
    kingdomId: values.kingdomId || "",
    profession: values.profession || "",
    objective: values.objective || "",
    fear: values.fear || "",
    playerLimits: values.playerLimits || "",
    currentLocation: values.currentLocation || "Limiar das Cortinas",
...
async function joinFaction(form) {
  if (!state.settings.factionsUnlocked) {
    toast("Os juramentos de facção ainda não foram abertos nesta temporada.");
    return;
  }
  const character = currentCharacter();
...
async function toggleEquip(instanceId) {
  ...
  const cosmeticGroup = /moldura|aura|perfil/.test(targetText) ? "frame" : /banner/.test(targetText) ? "banner" : "";
  const inventory = (character.inventory || []).map((item) => {
    const id = item.instanceId || item.id;
    if (id === instanceId) return { ...item, equipped: !item.equipped };
    if (!target?.equipped && cosmeticGroup) {
      const text = normalize(`${item.category || ""} ${item.categoryId || ""} ${item.name || ""}`);
      const group = /moldura|aura|perfil/.test(text) ? "frame" : /banner/.test(text) ? "banner" : "";
      if (group === cosmeticGroup) return { ...item, equipped: false };
    }
    return item;
  });
...
async function fuseVaultItem(instanceId) {
  ...
  if (!item) return;
  if (item.locked || item.equipped || petBusy(item)) {
    toast("Desbloqueie, desequipe e deixe o companheiro livre antes da fusão.");
    return;
  }
  ...
  const identity = normalize(item.sourceId || item.baseName || item.name);
  const duplicate = vault.find((entry) => entry.instanceId !== instanceId
    && normalize(entry.sourceId || entry.baseName || entry.name) === identity
...
function huntLootFor(biome = {}, difficulty = {}, pet = {}, cancelled = false) {
  ...
  const pools = {
    vidro: [{ id: "vidro-abissal", name: "Vidro abissal", rarity: "Raro" }, { id: "sal-lunar", name: "Sal lunar", rarity: "Incomum" }, { id: "estilhaco-vitreo", name: "Estilhaço vítreo", rarity: "Comum" }],
    pantano: [{ id: "bronze-funebre", name: "Bronze fúnebre", rarity: "Incomum" }, { id: "ervas-de-pantano", name: "Ervas de pântano", rarity: "Comum" }, { id: "perola-de-eco", name: "Pérola de Eco", rarity: "Raro" }],
    ferrugem: [{ id: "sucata-arcana", name: "Sucata arcana", rarity: "Comum" }, { id: "nucleo-opaco", name: "Núcleo opaco", rarity: "Raro" }, { id: "liga-ferruginea", name: "Liga ferrugínea", rarity: "Incomum" }],
    folhas: [{ id: "seiva-memorial", name: "Seiva memorial", rarity: "Incomum" }, { id: "madeira-viva", name: "Madeira viva", rarity: "Raro" }, { id: "semente-dourada", name: "Semente dourada", rarity: "Comum" }],
    carne: [{ id: "fibra-viva", name: "Fibra viva", rarity: "Raro" }, { id: "glandula-divina", name: "Glândula divina", rarity: "Épico" }, { id: "fruto-pulsante", name: "Fruto pulsante", rarity: "Incomum" }],
    cinzas: [{ id: "cinza-nominal", name: "Cinza nominal", rarity: "Incomum" }, { id: "pedra-juramento", name: "Pedra de juramento", rarity: "Raro" }, { id: "cinza-fria", name: "Cinza fria", rarity: "Comum" }],
    gelo: [{ id: "couro-palido", name: "Couro pálido", rarity: "Raro" }, { id: "galhada-lunar", name: "Galhada lunar", rarity: "Épico" }, { id: "gelo-eterno", name: "Gelo eterno", rarity: "Incomum" }],
    default: [{ id: "fibra-de-eco", name: "Fibra de Eco", rarity: "Comum" }, { id: "tecido-ritual", name: "Tecido ritual", rarity: "Incomum" }, { id: "cinza-fria", name: "Cinza fria", rarity: "Incomum" }],
  };
  const poolKey = biomeKey.includes("vidro") ? "vidro" : biomeKey.includes("pantano") || biomeKey.includes("afogad") ? "pantano" : biomeKey.includes("ferrugem") ? "ferrugem" : biomeKey.includes("folhas") || biomeKey.includes("bosque") ? "folhas" : biomeKey.includes("carne") ? "carne" : biomeKey.includes("cinza") || biomeKey.includes("nomes") ? "cinzas" : biomeKey.includes("frio") || biomeKey.includes("dente") ? "gelo" : "default";
  const materialPool = pools[poolKey];
  const multiplier = Math.max(1, Math.round(Number(difficulty.multiplier || 1)));
  const routeBonus = biome.huntRoute === "deep" ? 2 : biome.huntRoute === "cautious" ? -1 : 0;
  const quantity = Math.max(1, (cancelled ? 1 : 2) + routeBonus + Math.floor(multiplier / 1.4) + Math.floor(rarityScore(pet.rarity) / 3));
...
async function startPetHunt(form) {
  ...
  const requestedDuration = Math.max(15, Math.min(60, Number(values.durationMinutes || 15)));
  const route = values.route || "balanced";
  ...
  const activity = {
    ...
    loot: huntLootFor({ ...biome, huntRoute: route }, difficulty, pet, false).map((item) => `${item.name} x${item.quantity}`),
    events: [
      `Saiu por ${biome.name || "uma rota desconhecida"} em rota ${route === "deep" ? "profunda" : route === "cautious" ? "cautelosa" : "equilibrada"}`,
...
function routeLabel(type = "") {
  return ({ crossroads: "três ruas cruzadas", bridges: "pontes suspensas", ring: "uma arena circular", portals: "portais instáveis", citadel: "os andares da cidadela", marsh: "ilhas e marés", rails: "trilhos industriais", mirror: "duas rotas refletidas" })[type] || "uma rota desconhecida";
}
...
const TOWER_MAP_LAYOUTS = {
  ...
  "pantano-dos-sinos": {
    routes: [
      [[-24, 104], [106, 104], [174, 178], [300, 178], [380, 104], [510, 104], [588, 208], [784, 208]],
      [[-24, 332], [146, 332], [226, 254], [350, 254], [440, 332], [596, 332], [650, 246], [784, 246]],
    ],
    slots: [{ x: 106, y: 206 }, { x: 236, y: 104 }, { x: 314, y: 326 }, { x: 430, y: 210 }, { x: 548, y: 178 }, { x: 668, y: 318 }],
    accent: "#719283",
  },
  "ermos-da-ferrugem": {
    routes: [
      [[-24, 72], [178, 72], [246, 156], [390, 156], [466, 72], [784, 72]],
      [[-24, 214], [214, 214], [302, 214], [430, 214], [558, 214], [784, 214]],
      [[-24, 354], [160, 354], [252, 278], [416, 278], [510, 354], [784, 354]],
    ],
    slots: [{ x: 108, y: 146 }, { x: 160, y: 284 }, { x: 286, y: 100 }, { x: 354, y: 330 }, { x: 476, y: 142 }, { x: 604, y: 286 }, { x: 690, y: 142 }],
    accent: "#a9673d",
  },
  "mar-de-vidro-negro": {
    routes: [
      [[-24, 116], [112, 116], [202, 210], [336, 210], [426, 116], [560, 116], [650, 210], [784, 210]],
      [[-24, 316], [112, 316], [202, 222], [336, 222], [426, 316], [560, 316], [650, 222], [784, 222]],
    ],
    slots: [{ x: 122, y: 220 }, { x: 276, y: 122 }, { x: 366, y: 310 }, { x: 516, y: 214 }, { x: 660, y: 318 }],
    accent: "#6ca7bb",
  },
};
...
function towerLayoutFor(map = {}) {
  ...
  const fallbackId = ({ bridges: "aldeia-das-folhas-douradas", ring: "arena-das-sete-esferas", portals: "sociedade-das-laminas", citadel: "reino-do-pecado-partido", marsh: "pantano-dos-sinos", rails: "ermos-da-ferrugem", mirror: "mar-de-vidro-negro" })[map.routeType] || "cruzamento-das-cortinas";
...
function drawTowerEnemy(ctx, enemy, map = {}) {
  ...
  ctx.fillStyle = enemy.phased ? "rgba(153,126,231,.52)" : enemy.elite ? "#a8374d" : ({ bridges: "#726a54", ring: "#8b6e47", portals: "#67538d", citadel: "#70434b", marsh: "#496d61", rails: "#86583d", mirror: "#487b8d" })[type] || "#536474";
  ...
  if (type === "bridges") {
    ...
    ctx.ellipse(0, 0, radius * 1.15, radius * .8, 0, 0, Math.PI * 2); ctx.moveTo(-radius, -radius * .3); ctx.lineTo(-radius * .4, -radius * 1.25); ctx.lineTo(0, -radius * .5); ctx.lineTo(radius * .5, -radius * 1.2); ctx.lineTo(radius, -radius * .3);
  } else if (type === "rails") {
    ctx.moveTo(-radius, radius); ctx.lineTo(-radius, -radius); ctx.lineTo(-radius * .35, -radius * 1.35); ctx.lineTo(radius * .35, -radius * 1.35); ctx.lineTo(radius, -radius); ctx.lineTo(radius, radius); ctx.closePath();
  } else if (type === "mirror") {
    ctx.moveTo(0, -radius * 1.3); ctx.lineTo(radius, 0); ctx.lineTo(0, radius * 1.3); ctx.lineTo(-radius, 0); ctx.closePath(); ctx.moveTo(0, -radius); ctx.lineTo(0, radius);
  } else {
...
function openAuctionBid(auctionId) {
  ...
  if (!auction) return;
  const endTime = dateFromValue(auction.endsAt)?.getTime();
  if (!Number.isFinite(endTime) || endTime <= Date.now()) {
    toast("Este leilão não está aberto para novos lances.");
    return;
  }
...
async function sendGuildChat(form) {
  ...
  const messageId = await addDoc("guildMessages", payload);
  const pruneSource = state.guildMessages.some((message) => message.id === messageId)
    ? state.guildMessages
    : [...state.guildMessages, { ...payload, id: messageId, createdAt: new Date().toISOString() }];
  await pruneMessages("guildMessages", pruneSource, (message) => message.guildId === guild.id);
  form.reset();
}
...
async function deleteContentEntry(collection, id) {
  if (!collection || !id) return;
  const entry = state.content?.[collection]?.find((item) => item.id === id);
  const label = entry?.name || entry?.title || id;
  if (!window.confirm(`Excluir "${label}" da Interface? Esta ação remove a entrada publicada.`)) return;
  await deleteDoc(collection, id);
  if (state.content?.[collection]) state.content[collection] = state.content[collection].filter((item) => item.id !== id);
  toast(`${label} foi removido.`);
  render();
}
...
async function acceptTerms() {
  ...
  toast("Termo aceito. Bem-vindo à Interface.");
  render();
}
...
async function saveForgeVisual(form) {
  ...
  if (collection === "affinityCategories") {
    payload = { ...(existing || {}), name: values.name || "Nova categoria", imageUrl: values.imageUrl || "", weight: Math.max(0, Number(values.weight || 0)), rarity: values.rarity || "Comum", color: values.color || "#d8b45d", description: values.description || "" };
  }
  if (collection === "affinities") {
    payload = { ...(existing || {}), name: values.name || "Nova afinidade", imageUrl: values.imageUrl || "", categoryId: values.categoryId || "", bonus: forgeBonusFromValues(values), passive: values.passive || "", description: values.description || "" };
  }
  if (collection === "itemCategories") payload = { ...(existing || {}), name: values.name || "Nova categoria" };
  if (collection === "items") {
    payload = { ...(existing || {}), name: values.name || "Novo item", imageUrl: values.imageUrl || "", categoryId: values.categoryId || "", price: Math.max(0, Number(values.price || 0)), rarity: values.rarity || "Comum", bonus: forgeBonusFromValues(values), description: values.description || "" };
  }
  if (collection === "biomes") payload = { ...(existing || {}), name: values.name || "Novo bioma", imageUrl: values.imageUrl || "", region: values.region || "", description: values.description || "" };
  if (collection === "kingdoms") payload = { ...(existing || {}), name: values.name || "Novo reino", imageUrl: values.imageUrl || "", ruler: values.ruler || "", description: values.description || "" };
  if (collection === "regions") payload = { ...(existing || {}), name: values.name || "Nova região", imageUrl: values.imageUrl || "", kingdomId: values.kingdomId || "", description: values.description || "" };
  if (collection === "npcs") payload = { ...(existing || {}), name: values.name || "Novo NPC", imageUrl: values.imageUrl || "", role: values.role || "", region: values.region || "", description: values.description || "" };
  if (collection === "rulesChapters") payload = { ...(existing || {}), name: values.name || "Novo capítulo", order: Math.max(1, Number(values.order || 1)), summary: values.summary || "", full: values.full || "" };
  if (collection === "faqEntries") payload = { ...(existing || {}), name: values.name || "Nova pergunta", category: values.category || "FAQ", answer: values.answer || "" };
  if (collection === "tutorialSteps") payload = { ...(existing || {}), name: values.name || "Nova etapa", order: Math.max(0, Number(values.order || 0)), destination: values.destination || "", description: values.description || "", rule: values.rule || "" };
  if (collection === "worldLore") {
...
async function sendReward(form) {
  ...
  if (values.type === "gachaPet" || values.type === "gachaItem") {
    ...
    if (values.type === "gachaPet") patch.gachaVault = [...(character.gachaVault || []), { ...instance, source: `Prêmio ${ORACLE_LABEL}` }];
    else patch.inventory = [...(patch.inventory || character.inventory || []), { ...instance, categoryId: "especial", source: `Prêmio ${ORACLE_LABEL}` }];
    rewardName = instance.name;
    rewardType = values.type === "gachaPet" ? "pet de cofre" : "relíquia de inventário";
    pendingRewards.push(`${values.type === "gachaPet" ? "Pet de cofre" : "Relíquia de inventário"}: ${instance.name}`);
  }
...
async function sendAdminMail(form) {
  ...
  const gold = Math.max(0, Number(values.gold || 0));
  const coins = Math.max(0, Number(values.coins || 0));
  const energy = Math.max(0, Number(values.energy || 0));
  const fragmentAmount = Math.max(0, Number(values.fragmentAmount || 0));
  const fragmentName = values.fragmentName || "Fragmentos do Despertar";
  const rarity = values.rarity || "Comum";
  const item = values.itemId ? getItem(values.itemId) : null;
  const gachaPet = values.gachaPetId ? (state.content.gachaPets || []).find((entry) => entry.id === values.gachaPetId) : null;
  const gachaItem = values.gachaItemId ? (state.content.gachaItems || []).find((entry) => entry.id === values.gachaItemId) : null;
  ...
  const tokenName = String(values.tokenName || "").trim();
  const baseRewards = [];
  if (gold) baseRewards.push(`${gold} PO`);
  if (coins) baseRewards.push(`${coins} MC`);
  if (energy) baseRewards.push(`${energy} energia`);
  if (essences) baseRewards.push(`${essences} essência(s)`);
  if (fragmentAmount) baseRewards.push(`${fragmentAmount} ${fragmentName}`);
  if (titleName) baseRewards.push(`Título: ${titleName}`);
  if (tokenName) baseRewards.push(`Token: ${tokenName}`);
  if (item) baseRewards.push(`Item: ${item.name}`);
  if (gachaPet) baseRewards.push(`Companheiro: ${gachaPet.name}`);
  if (gachaItem) baseRewards.push(`Relíquia: ${gachaItem.name}`);
  ...
  for (const uid of targets) {
    ...
    const patch = {};
    if (gold) patch.gold = Number(character.gold || 0) + gold;
    if (coins) patch.millenniumCoins = Number(character.millenniumCoins || 0) + coins;
    if (energy) {
      patch.gachaEnergy = Math.min(GACHA_ENERGY_MAX, currentGachaEnergy(character) + energy);
      patch.gachaEnergyUpdatedAt = new Date().toISOString();
    }
    if (essences) patch.affinityAttempts = Number(character.affinityAttempts || 0) + essences;
    if (fragmentAmount) patch.gachaFragments = { ...(character.gachaFragments || {}), [fragmentName]: Number(character.gachaFragments?.[fragmentName] || 0) + fragmentAmount };
    if (titleName) {
      patch.titles = [...(character.titles || []), { id: `${slug(titleName)}-${cryptoRandom().slice(0, 6)}`, name: titleName, rarity, source: `Correio ${ORACLE_LABEL}` }];
    }
    if (tokenName) {
      patch.tokens = [...(character.tokens || []), { id: `${slug(tokenName)}-${cryptoRandom().slice(0, 6)}`, name: tokenName, rarity, source: `Correio ${ORACLE_LABEL}`, description: "Insígnia concedida pelo Oráculo." }];
    }
    if (item) {
      patch.inventory = [...(character.inventory || []), { ...item, instanceId: cryptoRandom(), equipped: false, source: `Correio ${ORACLE_LABEL}` }];
    }
    if (gachaPet) {
      patch.gachaVault = [...(character.gachaVault || []), { ...buildGachaInstance(gachaPet, "pets"), source: `Correio ${ORACLE_LABEL}` }];
    }
    if (gachaItem) {
      patch.inventory = [...(patch.inventory || character.inventory || []), { ...buildGachaInstance(gachaItem, "items"), categoryId: "especial", source: `Correio ${ORACLE_LABEL}` }];
    }
    if (affinity) {
      patch.affinityId = affinity.id;
    ...
    if (gachaPet) await announceRareReward(uid, gachaPet.name, gachaPet.rarity, "companheiro");
    if (gachaItem) await announceRareReward(uid, gachaItem.name, gachaItem.rarity, "relíquia");
    if (tokenName) await announceRareReward(uid, tokenName, rarity, "token");
...
function syncTowerDeckOptions(form) {
  if (!form) return;
  const selects = [...form.querySelectorAll("select[name^='towerPet']")];
...
function wireEvents() {
  ...
  document.addEventListener("click", async (event) => {
    ...
    try {
      ...
      if (action === "delete-content") await deleteContentEntry(button.dataset.collection, button.dataset.id);
      ...
      if (action === "open-cosmetics") {
        state.profileTab = "cosmetics";
        state.view = "profile";
        render();
      }
  ...
  document.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (type === "content-race") {
        ...
        await saveContent("races", { name: v.name, imageUrl: v.imageUrl || "", bonus: forgeBonusFromValues(v), passive: v.passive, description: v.description || "" });
        form.reset();
      }
      if (type === "content-class") {
        const v = formValues(form);
        await saveContent("classes", { name: v.name, imageUrl: v.imageUrl || "", bonus: forgeBonusFromValues(v), role: v.role, description: v.description || "" });
        form.reset();
      }
      ...
      if (type === "content-affinity") {
        const v = formValues(form);
        await saveContent("affinities", { name: v.name, categoryId: v.categoryId, imageUrl: v.imageUrl || "", bonus: forgeBonusFromValues(v), passive: v.passive, description: v.description || "" });
        form.reset();
      }
      ...
      if (type === "content-item") {
        const v = formValues(form);
        await saveContent("items", { name: v.name, categoryId: v.categoryId, imageUrl: v.imageUrl || "", price: Number(v.price || 0), rarity: v.rarity, bonus: forgeBonusFromValues(v), description: v.description || "" });
        form.reset();
      }
