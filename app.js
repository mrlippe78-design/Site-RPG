...
function renderMarket() {
  ...
  const filteredMarket = (state.content.marketListings || []).filter((item) => {
  ...
  const cards = {
    market: () => `
      <section class="market-catalog-tools">
        <div><p class="eyebrow">Catálogo oficial</p><h3>Equipamento para cada caminho</h3><p>${filteredMarket.length} de ${(state.content.marketListings || []).length} item(ns)</p></div>
